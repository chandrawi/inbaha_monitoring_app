...
interface BreadcrumbProps {
  ...
  children1: { name: string; text: string; }[];
  children2: { name: string; text: string; parent: string; }[];
  child1: string;
  child2: string;
};
...
export default function Breadcrumb(props: BreadcrumbProps) {
  ...
  const child1Text = () => {
    if (props.children1) {
      return props.children1.find((value) => value.name === props.child1)?.text;
    }
  };
  const child2Text = () => {
    if (props.children2) {
      return props.children2.find((value) => value.name === props.child2)?.text;
    }
  };

  const setLink = (menu: string, submenu: string ="", item: string = "") => {
  ...
  return (
    <div class="w-full px-2 pt-1.5 pb-2 flex flex-row items-center font-medium text-gray-800 dark:text-gray-200">
      ...
      <Show when={props.children2.length} fallback={
        ...
        <div tabindex="0" class="dropdown-content min-w-full absolute flex flex-col justify-center bg-white shadow-md shadow-slate-200 dark:shadow-slate-950 dark:bg-slate-800">
          ...
          {(item) => (
            <a href={setLink(props.parent.name, item.name)} class="w-full px-2 py-1 wrap-break-word hover:text-sky-800 border-t border-slate-200 border-dotted dark:hover:text-sky-300 dark:border-slate-700">
              {item.text}
            </a>
          )}
          </For>
        ...
        <Show when={props.children2.length}>
          <span class="icon-arrow_fill_right text-[0.75rem] text-sky-800 dark:text-sky-300 px-1"></span>
        </Show>
      </Show>

      <Show when={child1Text() && props.children2.length}>
        <div class="dropdown min-w-24 h-full relative bg-sky-100 dark:bg-sky-950 text-sm rounded-sm">
          <button tabindex="0" class="group w-full h-full px-2 py-0.5 flex flex-row items-center justify-between text-gray-800 hover:text-sky-800 dark:text-gray-50 dark:hover:text-sky-300">
          ...
          <div tabindex="0" class="dropdown-content min-w-full absolute flex flex-col justify-center bg-white shadow-md shadow-slate-200 dark:shadow-slate-950 dark:bg-slate-800">
            ...
            {(item) => (
              <Show when={item.parent == props.child1}>
                <a href={setLink(props.parent.name, item.parent, item.name)} class="w-full px-2 py-1 wrap-break-word hover:text-sky-800 border-t border-slate-200 border-dotted dark:hover:text-sky-300 dark:border-slate-700">
                  {item.name}
                </a>
              </Show>
            )}
